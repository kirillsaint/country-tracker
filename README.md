...
| `ORIZN_API_KEY` | ключ Orizn Visa API для справочника визовых режимов. Пусто — справочник выключен |
...
GET  /api/visa-info?country=XX&lang               режим въезда по каждому паспорту + подсказка правила
POST /api/visa-info/rule     { country, passportId, type?, limitDays?, windowDays?, lang }
GET  /api/visa-info/status                       состояние кэша справочника по паспортам
POST /api/visa-info/refresh  { passport }         принудительно обновить справочник
...
### Справочник визовых режимов (Orizn)

`visaInfo.ts`. Ключ — `ORIZN_API_KEY` (тариф Starter, 30 000 запросов/мес); без ключа справочник
выключен, условия задаются вручную. При добавлении паспорта сервер в фоне выкачивает все ~235
направлений (`/api/v1/visa`, коды ISO alpha-3, 4 параллельных запроса) в коллекцию `visa_cache`;
раз в 6 часов джоб обновляет пары старше недели. `GET /api/visa-info?country=XX` отдаёт по каждому
паспорту режим въезда, число дней, заметки и **подсказку правила** (`suggestRule`): справочник
даёт только число дней, тип выводится эвристикой из текста («90 in any 180» → rolling, «per
calendar year» → calendarYear, иначе «за въезд» с автосбросом) и помечен уверенностью.
`POST /api/visa-info/rule` создаёт правило безвиза для пары паспорт → страна (идемпотентно;
`documentRole: "visafree"`, `customized: true`, чтобы пересборка паспорта его не трогала);
с параметрами `type/limitDays/windowDays` — как подтвердил пользователь. Данные Orizn — подсказка,
в UI всегда виден источник и статус верификации.
